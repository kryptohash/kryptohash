...
{
    int64_t nBestHeight = g_signals.GetHeight().get_value_or(0);
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Kryptohash " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Kryptohash Core is probably already running."), addrBind.ToString());
        else
...
void RelayTransaction(const CTransaction& tx, const uint320& hash)
{
...
void RelayTransaction(const CTransaction& tx, const uint320& hash, const CDataStream& ss)
{
...
bool CAddrDB::Write(const CAddrMan& addr)
{
    ...
    ssPeers << addr;
    uint320 hash = Hash(ssPeers.begin(), ssPeers.end());
    ssPeers << hash;
...
bool CAddrDB::Read(CAddrMan& addr)
{
    ...
    uint320 hashTmp = Hash(ssPeers.begin(), ssPeers.end());
    if (hashIn != hashTmp)
