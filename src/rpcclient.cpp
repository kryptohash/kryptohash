...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    if (strMethod == "importprivkeyhex"       && n > 2) ConvertTo<bool>(params[2]);
...
std::string HelpMessageCli(bool mainProgram)
{
    ...
    strUsage += "  -rpcport=<port>        " + _("Connect to JSON-RPC on <port> (default: 38912 or testnet: 39424)") + "\n";
    ...
    if(mainProgram)
    {
        ...
        strUsage += "\n" + _("SSL options: (see the Kryptohash Wiki in the future for SSL setup instructions)") + "\n";
