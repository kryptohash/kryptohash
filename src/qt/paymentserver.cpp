...
const QString BITCOIN_IPC_PREFIX("kryptohash:");
const char* BITCOIN_REQUEST_MIMETYPE = "application/kryptohash-paymentrequest";
const char* BITCOIN_PAYMENTACK_MIMETYPE = "application/kryptohash-paymentack";
const char* BITCOIN_PAYMENTACK_CONTENTTYPE = "application/kryptohash-payment";
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start kryptohash: click-to-pay handler"));
        }
        else {
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
                ...
                emit message(tr("URI handling"),
                    tr("URI can not be parsed! This can be caused by an invalid Kryptohash address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
