...
const char* GetOpName(opcodetype opcode)
{
    ...
    case OP_SHA3_224               : return "OP_SHA3_224";
    case OP_SHA3_256               : return "OP_SHA3_256";
    case OP_SHAKE160               : return "OP_SHAKE160";
    case OP_SHAKE320               : return "OP_SHAKE320";
...
bool IsCanonicalPubKey(const valtype &vchPubKey, unsigned int flags) {
    ...
    if (vchPubKey.size() != 33)
        return error("Non-canonical public key: wrong size");
#else
    if (vchPubKey.size() < 33)
...
#ifdef USE_ED25519
    if (vchSig.size() != 105)
        return error("Non-canonical signature: incorrect size");
#else
    ...
    try
    {
        while (pc < pend)
        {
            ...
            switch (opcode)
            {
                ...
                //
                case OP_NOP:
                case OP_NOP5: case OP_NOP6: case OP_NOP7: case OP_NOP8: case OP_NOP9: case OP_NOP10:
                break;
                ...
                case OP_SHAKE160:
                {
                    ...
                    if (stack.size() < 1)
                        return false;
                    valtype& vch = stacktop(-1);
                    valtype vchHash((opcode == OP_RIPEMD160 || opcode == OP_SHA1 || opcode == OP_HASH160 || opcode == OP_SHAKE160) ? 20 : 32);
                    ...
                    else if (opcode == OP_SHA3_256)
                        SHA3_256(&vch[0], vch.size(), &vchHash[0]);
                    ...
                    {
                        uint256 hash = Hash256(vch.begin(), vch.end());
                        memcpy(&vchHash[0], &hash, sizeof(hash));
                    }
                    else if (opcode == OP_HASH160 || opcode == OP_SHAKE160)
                    {
                        uint160 hash160 = Shake160(vch);
                        memcpy(&vchHash[0], &hash160, sizeof(hash160));
                    }
                    popstack(stack);
                ...
                case OP_SHA3_224:
                {
                    ...
                    valtype& vch = stacktop(-1);
                    valtype vchHash(28);
                    uint224 hash = Hash224(vch.begin(), vch.end());
                    memcpy(&vchHash[0], &hash, sizeof(hash));
                    popstack(stack);
                    stack.push_back(vchHash);
                }
                ...
                case OP_SHAKE320:
                {
                    ...
                    valtype& vch = stacktop(-1);
                    valtype vchHash(40);
                    uint320 hash = Hash(vch.begin(), vch.end());
                    memcpy(&vchHash[0], &hash, sizeof(hash));
                    popstack(stack);
                    stack.push_back(vchHash);
                }
...
public:
    ...
    void Set(const uint256 &hash, const std::vector<unsigned char>& vchSig, const CPubKey& pubKey)
    {
        ...
        while (static_cast<int64_t>(setValid.size()) > nMaxCacheSize)
        {
            ...
            // than our cache size.
            uint256 randomHash = GetRandHash256();
...
{
    if (script.size() == 34 && script[0] == 32 && script[33] == OP_CHECKSIG) {
        pubkey.Set(&script[1], &script[33]);
        return true;
    }
