...
int HMAC_SHA3_512_Init(HMAC_SHA3_512_CTX *pctx, const void *pkey, size_t len)
{
    ...
    if (len <= 128) {
        ...
        Keccak_HashUpdate(&h, (BitSequence *)pkey, len);
        Keccak_HashFinal(&h, key);
        memset(key + 64, 0, 64);
    }
    ...
    Keccak_HashInitialize_SHA3_512(&pctx->hOuter);
    Keccak_HashUpdate(&pctx->hOuter, key, 128);

    for (int n = 0; n < 128; n++) {
        key[n] ^= 0x5c ^ 0x36;
    }
    Keccak_HashInitialize_SHA3_512(&pctx->hInner);

    return Keccak_HashUpdate(&pctx->hInner, key, 128);
}

int HMAC_SHA3_512_Update(HMAC_SHA3_512_CTX *pctx, const void *pdata, size_t len)
{
    return Keccak_HashUpdate(&pctx->hInner, (BitSequence *)pdata, len);
}
...
int HMAC_SHA3_512_Final(unsigned char *pmd, HMAC_SHA3_512_CTX *pctx)
{
    ...
    Keccak_HashFinal(&pctx->hInner, buf);
    Keccak_HashUpdate(&pctx->hOuter, buf, 64);

    return Keccak_HashFinal(&pctx->hOuter, pmd);
}
