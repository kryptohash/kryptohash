...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState &state, const CTransaction &tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee)
{
    ...
    {
        return state.DoS(100, error("AcceptToMemoryPool: : incorrect #coin"), REJECT_INVALID, "#coin");
    }
...
unsigned int ComputeMinWork(unsigned int nBase, int64_t nTime)
{
    ...
    if (TestNet() && nTime > 2 * nTargetSpacing * 1000) {
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if ((pindexLast->nHeight + 1) % nInterval != 0) {
        if (TestNet()) {
            ...
            if (pblock->nTxTime > pindexLast->nTxTime + 2 * nTargetSpacing * 1000) {
                return nProofOfWorkLimit;
            }
            else {
    ...
    int64_t nActualTimespan = pindexLast->GetBlockTxTime() - pindexFirst->GetBlockTxTime();
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) 
    {
        ...
        // Ignore commands with any #Coin for now. Future enhancement.
        if (hdr.nHashCoin) // && (hdr.nHashCoin & Params().GetHashCoinMask()) == 0)
        {
            continue;
        }
        string strCommand = hdr.GetCommand();
