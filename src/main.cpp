...
map<uint320, COrphanBlock*> mapOrphanBlocks;
multimap<uint320, COrphanBlock*> mapOrphanBlocksByPrev;

struct COrphanTx {
    CTransaction tx;
    NodeId fromPeer;
};
map<uint320, COrphanTx> mapOrphanTransactions;
...
const float PIDsetpoint     = (float)nTargetSpacing;
const float PIDproportional = 1.000f;
const float PIDintegral     = 0.050f;
...
const string strMessageMagic = "Kryptohash Signed Message:\n";
...
namespace {
	...
	map<uint320, NodeId> mapBlockSource;
    ...
    map<uint320, pair<NodeId, list<QueuedBlock>::iterator> > mapBlocksInFlight;
    map<uint320, pair<NodeId, list<uint320>::iterator> > mapBlocksToDownload;
}
...
struct CMainSignals {
    ...
    boost::signals2::signal<void (const uint320 &, const CTransaction &, const CBlock *)> SyncTransaction;
    // Notifies listeners of an erased transaction (currently disabled, requires transaction replacement).
    boost::signals2::signal<void (const uint320 &)> EraseTransaction;
    // Notifies listeners of an updated transaction without new data (for now: a coinbase potentially becoming visible).
    boost::signals2::signal<void (const uint320 &)> UpdatedTransaction;
    // Notifies listeners of a new active block chain.
    boost::signals2::signal<void (const CBlockLocator &)> SetBestChain;
    // Notifies listeners about an inventory item being seen on the network.
    boost::signals2::signal<void (const uint320 &)> Inventory;
...
void SyncWithWallets(const uint320 &hash, const CTransaction &tx, const CBlock *pblock) {
...
namespace {
	...
	struct CNodeState {
	    ...
	    int nBlocksInFlight;
	    list<uint320> vBlocksToDownload;
	...
	void FinalizeNode(NodeId nodeid) {
	    ...
	    BOOST_FOREACH(const uint320& hash, state->vBlocksToDownload)
	...
	// Requires cs_main.
	void MarkBlockAsReceived(const uint320 &hash, NodeId nodeFrom = -1) {
	    map<uint320, pair<NodeId, list<uint320>::iterator> >::iterator itToDownload = mapBlocksToDownload.find(hash);
	    ...
	    map<uint320, pair<NodeId, list<QueuedBlock>::iterator> >::iterator itInFlight = mapBlocksInFlight.find(hash);
	...
	// Requires cs_main.
	bool AddBlockToQueue(NodeId nodeid, const uint320 &hash) {
	    ...
	    list<uint320>::iterator it = state->vBlocksToDownload.insert(state->vBlocksToDownload.end(), hash);
	    state->nBlocksToDownload++;
	...
	// Requires cs_main.
	void MarkBlockAsInFlight(NodeId nodeid, const uint320 &hash) {
	    CNodeState *state = State(nodeid);
	    assert(state != NULL);
...
CBlockLocator CChain::GetLocator(const CBlockIndex *pindex) const {
    ...
    while (pindex) {
        ...
        int64_t nHeight = std::max(pindex->nHeight - nStep, (int64_t)0);
...
CBlockIndex *CChain::FindFork(const CBlockLocator &locator) const {
    ...
    BOOST_FOREACH(const uint320& hash, locator.vHave) {
        std::map<uint320, CBlockIndex*>::iterator mi = mapBlockIndex.find(hash);
...
void static EraseOrphanTx(uint320 hash)
{
    map<uint320, COrphanTx>::iterator it = mapOrphanTransactions.find(hash);
    if (it == mapOrphanTransactions.end()) {
        return;
    }
    BOOST_FOREACH(const CTxIn& txin, it->second.tx.vin)
    {
        map<uint320, set<uint320> >::iterator itPrev = mapOrphanTransactionsByPrev.find(txin.prevout.hash);
...
{
    int nErased = 0;
    map<uint320, COrphanTx>::iterator iter = mapOrphanTransactions.begin();
    while (iter != mapOrphanTransactions.end())
    {
        map<uint320, COrphanTx>::iterator maybeErase = iter++; // increment to avoid iterator becoming invalid
...
unsigned int LimitOrphanTxSize(unsigned int nMaxOrphans)
{
    ...
    while (mapOrphanTransactions.size() > nMaxOrphans)
    {
        ...
        uint320 randomhash = GetRandHash();
        map<uint320, COrphanTx>::iterator it = mapOrphanTransactions.lower_bound(randomhash);
        if (it == mapOrphanTransactions.end()) {
...
bool IsFinalTx(const CTransaction &tx, int64_t nBlockHeight, int64_t nBlockTime)
{
    AssertLockHeld(cs_main);
    ...
    if (tx.nLockTime < (tx.nLockTime < LOCKTIME_THRESHOLD ? nBlockHeight : nBlockTime)) {
        return true;
    }
    BOOST_FOREACH(const CTxIn& txin, tx.vin) {
        if (!txin.IsFinal()) {
...
int64_t CMerkleTx::SetMerkleBranch(const CBlock* pblock)
{
    AssertLockHeld(cs_main);
    CBlock blockTmp;
    ...
    if (pblock) {
        ...
        hashBlock = pblock->GetKryptoHash();
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state)
{
    ...
    if (tx.IsCoinBase() || tx.IsCoinShare()) {
        if (tx.vin[0].scriptSig.size() < 2 || tx.vin[0].scriptSig.size() > 100) {
            return state.DoS(100, error("CheckTransaction() : coinbase script size"), REJECT_INVALID, "bad-cb-length");
        }
    }
    else {
        BOOST_FOREACH(const CTxIn& txin, tx.vin) {
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState &state, const CTransaction &tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee)
{
    ...
    // Coinshare is only valid when it is from the Genesis block, not as a loose transaction
    if (tx.IsCoinShare() && tx.vin[0].prevout.hash != Params().GenesisMerkleRoot()) {
...
{
    if (!IsCoinBase() || chainActive.Height() < 100) {
        return 0;
    }
    ...
    CBlockIndex *pindexSlow = NULL;
    {
        ...
        if (fTxIndex) {
            ...
            if (pblocktree->ReadTxIndex(hash, postx)) {
                ...
                hashBlock = header.GetKryptoHash();
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, bool fReadTransactions)
{
    block.SetNull();
    ...
    // Check the header
    if (!CheckProofOfWork(block.GetKryptoHash(), block.nBits)) {
...
bool ReadBlockFromDisk(CBlock& block, const CBlockIndex* pindex)
{
    ...
    if (block.GetKryptoHash() != pindex->GetBlockHash()) {
        return error("ReadBlockFromDisk(CBlock&, CBlockIndex*) : GetHash() doesn't match index");
    }
    return true;
}

uint320 static GetOrphanRoot(const uint320& hash)
{
    map<uint320, COrphanBlock*>::iterator it = mapOrphanBlocks.find(hash);
    if (it == mapOrphanBlocks.end()) {
        return hash;
    }
    // Work back to the first block in the orphan chain
    do {
        map<uint320, COrphanBlock*>::iterator it2 = mapOrphanBlocks.find(it->second->hashPrev);
...
void static PruneOrphanBlocks()
{
    ...
    std::multimap<uint320, COrphanBlock*>::iterator it = mapOrphanBlocksByPrev.begin();
    while (pos--) it++;

    // As long as this block has other orphans depending on it, move to one of those successors.
    do {
        std::multimap<uint320, COrphanBlock*>::iterator it2 = mapOrphanBlocksByPrev.find(it->second->hashBlock);
    ...
    uint320 hash = it->second->hashBlock;
    delete it->second;
...
unsigned int ComputeMinWork(unsigned int nBase, int64_t nTime)
{
    ...
    // after 2*nTargetSpacing time between blocks:
    if (TestNet() && nTime > 2 * nTargetSpacing) {
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if ((pindexLast->nHeight + 1) % nInterval != 0) {
        if (TestNet()) {
            ...
            if (pblock->nTime > pindexLast->nTime + 2 * nTargetSpacing) {
                return nProofOfWorkLimit;
            }
            else {
...
//
unsigned int ComputeMinWorkPID(unsigned int nBase, int64_t nTxTime)
{
    // Sanity check
    int64_t deltaTime = nTxTime - PIDCheckpoints::PIDGetTimeLastCheckpoint();
    if (deltaTime < 0) {
        LogPrintf("ComputeMinWorkPID: block with timestamp before last PID checkpoint");
        return 0;
    }
    ...
    if (nLastPIDCheckpoint == 0) {
        while (deltaTime > 0 && bnResult < bnLimit)
        {
            ...
            bnResult *= 4;
            // ... in best-case exactly 4-times-normal target time
            deltaTime -= 4 * nTargetTimespan * 1000;
        }
    }
    else {
        ...
        CPID myPID = *PID;
        if (myPID.GetDiff() == 0 || myPID.SetPoint == 0.0f) {
            LogPrintf("ComputeMinWorkPID: Invalid PID Checkpoint");
            return 0;
        }

        CBigNum bnPIDcheckpoint;
        bnPIDcheckpoint.SetCompact(myPID.GetDiff());
...
//
unsigned int CalcRetarget(const unsigned int nBits, const float delta)
{
    unsigned int nExp = (nBits & 0xFF000000) >> 24;
    unsigned int nbits = nBits & 0x7FFFFF;

    if (delta < 0) {
        unsigned int nDelta = (unsigned int)(abs(delta)) * 0x100;
        ...
        unsigned int nDelta = (unsigned int)delta * 0x100;
...
void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip)
{
    ...
    if (pfork && (!pindexBestForkTip || (pindexBestForkTip && pindexNewForkTip->nHeight > pindexBestForkTip->nHeight)) &&
            pindexNewForkTip->nChainWork - pfork->nChainWork > (pfork->GetBlockWork() * 7).getuint320() &&
...
void static InvalidBlockFound(CBlockIndex *pindex, const CValidationState &state) {
    ...
    if (state.IsInvalid(nDoS)) {
        std::map<uint320, NodeId>::iterator it = mapBlockSource.find(pindex->GetBlockHash());
...
{
    int64_t maxTimeElapsed = max(GetTimeMillis(), GetAdjustedTime() * 1000);

    // Subtract nTxTime which is the time when the transaction was created (in ms).
    block.nTime = (maxTimeElapsed - block.nTxTime);
#if 0
    // Updating time can change work required on testnet:
    if (TestNet()) {
        block.nBits = GetNextWorkRequired(pindexPrev, &block);
...
void UpdateCoins(const CTransaction& tx, CValidationState &state, CCoinsViewCache &inputs, CTxUndo &txundo, int64_t nHeight, const uint320 &txhash)
{
    bool ret;
    // mark inputs spent
    if (!tx.IsCoinBase()) {
...
bool CheckInputs(const CTransaction& tx, CValidationState &state, CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, std::vector<CScriptCheck> *pvChecks)
{
    if (!tx.IsCoinBase()) {
        ...
        int64_t nSpendHeight = pindexPrev->nHeight + 1;
        int64_t nValueIn = 0;
        int64_t nFees = 0;
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
        ...
        if (nValueIn < tx.GetValueOut()) {
            return state.DoS(100, error("CheckInputs() : %s value in < value out", tx.GetHash().ToString()), REJECT_INVALID, "bad-txns-in-belowout");
        }
        ...
        // Only if ALL inputs pass do we perform expensive ECDSA or ed25519 signature checks.
        // Helps prevent CPU exhaustion attacks.

        // Skip ECDSA/ed25519 signature verification when connecting blocks
...
void ThreadScriptCheck() {
    RenameThread("kryptohash-scriptch");
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    uint320 hashPrevBlock = pindex->pprev == NULL ? uint320(0) : pindex->pprev->GetBlockHash();
    ...
    if (!pindex->phashBlock) {
        ...
        {
            uint320 hash = block.GetTxHash(i);
            if (view.HaveCoins(hash) && !view.GetCoins(hash).IsPruned()) {
                return state.DoS(100, error("ConnectBlock() : tried to overwrite transaction"), REJECT_INVALID, "bad-txns");
            }
        }
    }
...
void static UpdateTip(CBlockIndex *pindexNew) 
{
    ...
    if ((chainActive.Height() % nInterval) == 0 || (!fIsInitialDownload && (chainActive.Height() % 144) == 0)) {
...
bool AddToBlockIndex(CBlock& block, CValidationState& state, const CDiskBlockPos& pos)
{
    ...
    map<uint320, CBlockIndex*>::iterator miPrev = mapBlockIndex.find(block.hashPrevBlock);
    if (miPrev != mapBlockIndex.end()) {
        pindexNew->pprev = (*miPrev).second;
        pindexNew->nHeight = pindexNew->pprev->nHeight + 1;
    }
    pindexNew->nTx = block.vtx.size();
    pindexNew->nChainWork = (pindexNew->pprev ? pindexNew->pprev->nChainWork : 0) + pindexNew->GetBlockWork().getuint320();
    ...
    if (pindexNew == chainActive.Tip()) {
        ...
        static uint320 hashPrevBestCoinBase;
...
bool FindBlockPos(CValidationState &state, CDiskBlockPos &pos, unsigned int nAddSize, int64_t nHeight, uint64_t nTime, bool fKnown = false)
{
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (hash == Params().HashGenesisBlock()) {
        ...
        if (block.vtx.size() == 1) {
            ...
            if (!std::equal(block1.begin(), block1.end(), block.vtx[0].vin[0].scriptSig.begin()) ) {
    ...
    for (unsigned int i = 1; i < block.vtx.size(); i++) {
        if (block.vtx[i].IsCoinBase() || block.vtx[i].IsCoinShare()) {
            return state.DoS(100, error("CheckBlock() : more than one coinbase/coinshare"), REJECT_INVALID, "bad-cb-cs-multiple");
        }
    }
    // Check transactions
    BOOST_FOREACH(const CTransaction& tx, block.vtx) {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CDiskBlockPos* dbp)
{
    ...
    int64_t nHeight = 0;
    if (hash != Params().HashGenesisBlock()) {
        map<uint320, CBlockIndex*>::iterator mi = mapBlockIndex.find(block.hashPrevBlock);
        if (mi == mapBlockIndex.end()) {
        ...
        if (!Checkpoints::CheckBlock(nHeight, hash)) {
            return state.DoS(100, error("AcceptBlock() : rejected by checkpoint lock-in at %d", nHeight), REJECT_CHECKPOINT, "checkpoint mismatch");
        }
        ...
        // Check proof of work
#if 1
        uint32_t nRequiredDiff = GetNextWorkRequired(pindexPrev, &block);
#else
    ...
    try {
        ...
        if (!FindBlockPos(state, blockPos, nBlockSize + 8, nHeight, block.nTxTime, dbp != NULL)) {
    ...
    int64_t nBlockEstimate = Checkpoints::GetTotalBlocksEstimate();
...
void PushGetBlocks(CNode* pnode, CBlockIndex* pindexBegin, uint320 hashEnd)
{
    AssertLockHeld(cs_main);
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    if (pcheckpoint && pblock->hashPrevBlock != (chainActive.Tip() ? chainActive.Tip()->GetBlockHash() : uint320(0))) {
        ...
        int64_t deltaTime = pblock->GetBlockTime() + nTargetTimespan - pcheckpoint->nTime; // Allow extra 24 hours since last checkpoint.
        if (deltaTime < 0) {
    ...
    vector<uint320> vWorkQueue;
    vWorkQueue.push_back(hash);
    for (unsigned int i = 0; i < vWorkQueue.size(); i++)
    {
        uint320 hashPrev = vWorkQueue[i];
        for (multimap<uint320, COrphanBlock*>::iterator mi = mapOrphanBlocksByPrev.lower_bound(hashPrev); mi != mapOrphanBlocksByPrev.upper_bound(hashPrev); ++mi)
        {
            CBlock block;
            {
...
CMerkleBlock::CMerkleBlock(const CBlock& block, CBloomFilter& filter)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        uint320 hash = block.vtx[i].GetHash();
...
uint320 CPartialMerkleTree::CalcHash(int height, unsigned int pos, const std::vector<uint320> &vTxid) {
    if (height == 0) {
        // hash at height 0 is the txids themself
        return vTxid[pos];
    } else {
        // calculate left hash
        uint320 left = CalcHash(height-1, pos*2, vTxid), right;
...
void CPartialMerkleTree::TraverseAndBuild(int height, unsigned int pos, const std::vector<uint320> &vTxid, const std::vector<bool> &vMatch) {
...
uint320 CPartialMerkleTree::TraverseAndExtract(int height, unsigned int pos, unsigned int &nBitsUsed, unsigned int &nHashUsed, std::vector<uint320> &vMatch) {
    ...
    if (height==0 || !fParentOfMatch) {
        ...
        const uint320 &hash = vHash[nHashUsed++];
        if (height==0 && fParentOfMatch) // in case of height 0, we have a matched txid
            vMatch.push_back(hash);
        return hash;
    } else {
        // otherwise, descend into the subtrees to extract matched txids and hashes
        uint320 left = TraverseAndExtract(height-1, pos*2, nBitsUsed, nHashUsed, vMatch), right;
        if (pos*2+1 < CalcTreeWidth(height-1))
...
CPartialMerkleTree::CPartialMerkleTree(const std::vector<uint320> &vTxid, const std::vector<bool> &vMatch) : nTransactions(vTxid.size()), fBad(false) {
    // reset state
    vBits.clear();
    vHash.clear();
...
uint320 CPartialMerkleTree::ExtractMatches(std::vector<uint320> &vMatch) {
    vMatch.clear();
    ...
    uint320 hashMerkleRoot = TraverseAndExtract(nHeight, 0, nBitsUsed, nHashUsed, vMatch);
...
CBlockIndex * InsertBlockIndex(uint320 hash)
{
    if (hash == 0) {
        return NULL;
    }
    // Return existing
    map<uint320, CBlockIndex*>::iterator mi = mapBlockIndex.find(hash);
    if (mi != mapBlockIndex.end()) {
        return (*mi).second;
    }
    // Create new
...
bool static LoadBlockIndexDB()
{
    ...
    vector<pair<int64_t, CBlockIndex*> > vSortedByHeight;
    vSortedByHeight.reserve(mapBlockIndex.size());
    BOOST_FOREACH(const PAIRTYPE(uint320, CBlockIndex*)& item, mapBlockIndex)
    {
        CBlockIndex* pindex = item.second;
        vSortedByHeight.push_back(make_pair(pindex->nHeight, pindex));
    }
    sort(vSortedByHeight.begin(), vSortedByHeight.end());
    BOOST_FOREACH(const PAIRTYPE(int64_t, CBlockIndex*)& item, vSortedByHeight)
    {
        CBlockIndex* pindex = item.second;
        pindex->nChainWork = (pindex->pprev ? pindex->pprev->nChainWork : 0) + pindex->GetBlockWork().getuint320();
    ...
    std::map<uint320, CBlockIndex*>::iterator it = mapBlockIndex.find(pcoinsTip->GetBestBlock());
...
bool VerifyDB(int nCheckLevel, int64_t nCheckDepth)
{
    LOCK(cs_main);
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        try {
            ...
            unsigned int nBlockSize = ::GetSerializeSize(block, SER_DISK, CLIENT_VERSION);
            nBlockSize += 8; // room for pchMessageStart and block size.
            CDiskBlockPos blockPos;
            CValidationState state;
            if (!FindBlockPos(state, blockPos, nBlockSize, 0, block.nTxTime)) {
...
void PrintBlockTree()
{
    ...
    for (map<uint320, CBlockIndex*>::iterator mi = mapBlockIndex.begin(); mi != mapBlockIndex.end(); ++mi)
    {
        CBlockIndex* pindex = (*mi).second;
...
void static ProcessGetData(CNode* pfrom)
{
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            {
                bool send = false;
                map<uint320, CBlockIndex*>::iterator mi = mapBlockIndex.find(inv.hash);
                if (mi != mapBlockIndex.end())
                {
                    // If the requested block is at a height below our last
                    // checkpoint, only serve it if it's in the checkpointed chain
                    int64_t nHeight = mi->second->nHeight;
                    CBlockIndex* pcheckpoint = Checkpoints::GetLastCheckpoint(mapBlockIndex);
                    if (pcheckpoint && nHeight < pcheckpoint->nHeight) {
                ...
                if (send)
                {
                    ...
                    else // MSG_FILTERED_BLOCK)
                    {
                        ...
                        if (pfrom->pfilter)
                        {
                            ...
                            typedef std::pair<unsigned int, uint320> PairType;
...
bool static ProcessMessage(CNode* pfrom, CMessageHeader& hdr, CDataStream& vRecv)
{
    RandAddSeedPerfmon();
    ...
    if (strCommand == "version") {
        ...
        if (pfrom->cleanSubVer.find("/Kryptohatoshi:0.") != std::string::npos) {
            LogPrintf("Client %s runs obsolete version < 1.0.0, disconnecting\n", pfrom->addr.ToString());
            pfrom->fDisconnect = true;
            return false;
        }
        if (!vRecv.empty()) {
        ...
        if ( fServer && !MainNet() ) {
            static bool askforMemPool = true;
            if (askforMemPool && !mempool.size()) {
        ...
        BOOST_FOREACH(CAddress& addr, vAddr)
        {
            ...
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable())
            {
                // Relay to a limited number of other nodes
                {
                    ...
                    if (hashSalt == 0)
                        hashSalt = GetRandHash256();
                    uint64_t hashAddr = addr.GetHash();
                    uint256 hashRand = hashSalt ^ (hashAddr<<32) ^ ((GetTime()+hashAddr)/(24*60*60));
                    hashRand = Hash256(BEGIN(hashRand), END(hashRand));
                    ...
                    BOOST_FOREACH(CNode* pnode, vNodes)
                    {
                        ...
                        hashKey = Hash256(BEGIN(hashKey), END(hashKey));
    ...
    else if (strCommand == "getheaders")
    {
        ...
        if (locator.IsNull())
        {
            ...
            map<uint320, CBlockIndex*>::iterator mi = mapBlockIndex.find(hashStop);
            if (mi == mapBlockIndex.end())
                return true;
            pindex = (*mi).second;
        }
    ...
    else if (strCommand == "tx")
    {
        ...
        if (AcceptToMemoryPool(mempool, state, tx, true, &fMissingInputs))
        {
            ...
            for (unsigned int i = 0; i < vWorkQueue.size(); i++)
            {
                map<uint320, set<uint320> >::iterator itByPrev = mapOrphanTransactionsByPrev.find(vWorkQueue[i]);
                if (itByPrev == mapOrphanTransactionsByPrev.end()) {
                    continue;
                }
                for (set<uint320>::iterator mi = itByPrev->second.begin(); mi != itByPrev->second.end(); ++mi)
                {
                    const uint320& orphanHash = *mi;
            ...
            BOOST_FOREACH(uint320 hash, vEraseQueue)
                EraseOrphanTx(hash);
        }
    ...
    else if (strCommand == "block" && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        LogPrint("net", "received block %s\n", block.GetKryptoHash().ToString());
        // block.print();

        CInv inv(MSG_BLOCK, block.GetKryptoHash());
        pfrom->AddInventoryKnown(inv);
    ...
    else if (strCommand == "mempool")
    {
        ...
        std::vector<uint320> vtxid;
        mempool.queryHashes(vtxid);
        vector<CInv> vInv;
        BOOST_FOREACH(uint320& hash, vtxid) {
            CInv inv(MSG_TX, hash);
            CTransaction tx;
            bool fInMemPool = mempool.lookup(hash, tx);
            if (!fInMemPool) 
                continue; // another thread removed since queryHashes, maybe...
            if ((pfrom->pfilter && pfrom->pfilter->IsRelevantAndUpdate(tx, hash)) || (!pfrom->pfilter))
                vInv.push_back(inv);
            if (vInv.size() == MAX_INV_SZ) {
                pfrom->PushMessage("inv", vInv);
                vInv.clear();
    ...
    else if (strCommand == "alert")
    {
        ...
        uint256 alertHash = alert.GetHash256();
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) 
    {
        ...
        uint256 hash = Hash256(vRecv.begin(), vRecv.begin() + nMessageSize);
        ...
        try
        {
            fRet = ProcessMessage(pfrom, hdr, vRecv);
            boost::this_thread::interruption_point();
        }
...
{
    {
        ...
        if (pto->fStartSync && !fImporting && !fReindex) {
            ...
            PushGetBlocks(pto, chainActive.Tip(), uint320(0));
        }
        ...
        vector<CInv> vInvWait;
        {
            ...
            BOOST_FOREACH(const CInv& inv, pto->vInventoryToSend)
            {
                ...
                if (inv.type == MSG_TX && !fSendTrickle)
                {
                    ...
                    static uint320 hashSalt;
                    if (hashSalt == 0)
                        hashSalt = GetRandHash();
                    uint320 hashRand = inv.hash ^ hashSalt;
        ...
        while (!pto->fDisconnect && state.nBlocksToDownload && state.nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            uint320 hash = state.vBlocksToDownload.front();
...
public:
    ...
    ~CMainCleanup() {
        // block headers
        std::map<uint320, CBlockIndex*>::iterator it1 = mapBlockIndex.begin();
        for (; it1 != mapBlockIndex.end(); it1++)
            delete (*it1).second;
        mapBlockIndex.clear();

        // orphan blocks
        std::map<uint320, COrphanBlock*>::iterator it2 = mapOrphanBlocks.begin();
