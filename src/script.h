...
// Copyright (c) 2014 Krytohash developers
...
public:
    ...
    CScript& operator<<(const uint320& b)
    {
        insert(end(), sizeof(b));
        insert(end(), (unsigned char*)&b, (unsigned char*)&b + sizeof(b));
        return *this;
    ...
    {
        return CScriptID(Shake160(*this));
    }
};
...
public:
    ...
    void Serialize(Stream &s, int nType, int nVersion) const {
        ...
        s << VARINT(nSize);
#ifdef _MSC_VER
        s << CFlatData(&script[0], &script[0] + script.size());
#else
    ...
    void Unserialize(Stream &s, int nType, int nVersion) {
        ...
        script.resize(nSize);
#ifdef _MSC_VER
        s >> REF(CFlatData(&script[0], &script[0] + script.size()));
#else
